...
- (void)setupMemorySwitch {
    ...
    sw.on = [OnyxPrefsRead(@"MemoryEnabled") boolValue] ?: YES;
...
- (void)memorySwitchChanged:(UISwitch *)sw {
    OnyxPrefsWrite(@"MemoryEnabled", @(sw.on));
    // 立即应用：开启=恢复上次模拟状态；关闭=停止模拟（下次进入也停）
    BOOL running = sw.on && [OnyxPrefsRead(@"enabled") boolValue];
    OnyxPrefsWrite(@"enabled", @(running));
    // 通知 MapVC 刷新界面
